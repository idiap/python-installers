...
func testBuild(t *testing.T, context spec.G, it spec.S) {
	...
	it("returns a result that installs uv", func() {
		...
		Expect(layer.Name).To(Equal("uv"))
		Expect(layer.Path).To(Equal(filepath.Join(layersDir, "uv")))
		...
		Expect(layer.Metadata["dependency-sha"]).To(Equal("uv-dependency-sha"))
		...
		Expect(dependencyManager.ResolveCall.Receives.Id).To(Equal("uv"))
		...
		Expect(dependencyManager.DeliverCall.Receives.DestinationPath).To(Equal(filepath.Join(layersDir, "uv-temp-layer")))
		Expect(dependencyManager.DeliverCall.Receives.PlatformPath).To(Equal("some-platform-path"))

		Expect(sbomGenerator.GenerateFromDependencyCall.Receives.Dir).To(Equal(filepath.Join(layersDir, "uv")))

		Expect(buffer.String()).To(ContainSubstring("Some Buildpack some-version"))
		Expect(buffer.String()).To(ContainSubstring("Executing build process"))
		Expect(buffer.String()).To(ContainSubstring("Installing uv"))
	})

	context("when the uv layer is required at build and launch", func() {
		it.Before(func() {
	...
	context("failure cases", func() {
		...
		context("when the layer dir cannot be reset", func() {
			it.Before(func() {
				Expect(os.MkdirAll(filepath.Join(layersDir, "uv", "bin"), os.ModePerm)).To(Succeed())
				Expect(os.Chmod(filepath.Join(layersDir, "uv"), 0500)).To(Succeed())
			})

			it.After(func() {
				Expect(os.Chmod(filepath.Join(layersDir, "uv"), os.ModePerm)).To(Succeed())
			})
