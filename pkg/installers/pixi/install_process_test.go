...
import (
	...
	"github.com/paketo-buildpacks/python-package-managers-install/pkg/installers/pixi"
)

func testPixiInstallProcess(t *testing.T, context spec.G, it spec.S) {
	var (
		...
		installProcess pixi.PixiInstallProcess
	)
	...
	context("Calling Execute", func() {
		...
		for _, arch := range archs {
			context(fmt.Sprintf("Test for %s", arch), func() {
				it.Before(func() {
					archFolder := filepath.Join(srcDir, fmt.Sprintf(pixi.PixiArchiveTemplateName, installProcess.TranslateArchitecture(arch)))
